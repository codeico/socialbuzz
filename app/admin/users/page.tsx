...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    loadUsers();
    loadStats();
  }, [pagination.page, searchQuery, filterRole, filterStatus]);
                  ...
                  {users.map((user) => (
                    <tr key={user.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-4">
                        <div className="flex items-center">
                          {user.is_verified ? (
                            <CheckCircle className={`h-4 w-4 mr-1 ${getStatusColor(true)}`} />
                          ) : (
                            <XCircle className={`h-4 w-4 mr-1 ${getStatusColor(false)}`} />
                          )}

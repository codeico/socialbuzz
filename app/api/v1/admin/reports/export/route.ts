...
async function generateRevenueReport(startDate: string, endDate: string): Promise<string> {
  ...
  data?.forEach(transaction => {
    ...
    const fromUser = transaction.user?.username || 'Unknown';
    const toUser = transaction.recipient?.username || 'N/A';
...
async function generateUsersReport(startDate: string, endDate: string): Promise<string> {
  const { data } = await supabaseAdmin
    .from('users')
    .select('id, username, email, full_name, role, balance, total_earnings, total_donations, is_verified, created_at, last_login')
    .gte('created_at', startDate)
    .lt('created_at', endDate)
    .order('created_at', { ascending: false });

  let csv = 'User ID,Username,Email,Full Name,Role,Balance,Total Earnings,Total Donations,Verified,Created Date,Last Login\n';
...
async function generateTransactionsReport(startDate: string, endDate: string): Promise<string> {
  ...
  let csv = 'Transaction ID,Type,Amount,Currency,Status,Payment Method,Reference ID,Created Date,Completed Date,From User,From Email,To User,To Email\n';
  
  data?.forEach(transaction => {
    const createdDate = new Date(transaction.created_at).toISOString().split('T')[0];
    const completedDate = transaction.completed_at ? new Date(transaction.completed_at).toISOString().split('T')[0] : 'N/A';
    const fromUser = transaction.user?.username || 'Unknown';
    const fromEmail = transaction.user?.email || 'Unknown';
    const toUser = transaction.recipient?.username || 'N/A';
    const toEmail = transaction.recipient?.email || 'N/A';
...
async function generateCreatorsReport(startDate: string, endDate: string): Promise<string> {
  ...
  let csv = 'Creator ID,Username,Email,Full Name,Total Earnings,Total Donations Made,Current Balance,Verified,Created Date\n';
...
async function generateOverviewReport(startDate: string, endDate: string): Promise<string> {
  ...
  const averageTransaction = totalTransactions && totalTransactions > 0 
